...
func TestHasTrackedChanges(t *testing.T) {
	t.Run("returns false for clean repo", func(t *testing.T) {
		...
		hasChanges, err := HasTrackedChanges(tempDir)
		if err != nil {
			t.Fatalf("HasTrackedChanges failed: %v", err)
		}
		if hasChanges {
			t.Error("expected clean repo to have no tracked changes")
		}
	})

	t.Run("returns false for only untracked files", func(t *testing.T) {
		tempDir := t.TempDir()
		...
		if err := os.WriteFile(untrackedFile, []byte("content"), fs.FileStrict); err != nil {
		...
		if hasChanges {
			t.Error("expected untracked files only to return false")
		}
	})
	...
	t.Run("returns true for staged files", func(t *testing.T) {
		...
		if err := os.WriteFile(stagedFile, []byte("content"), fs.FileStrict); err != nil {
		...
		if !hasChanges {
			t.Error("expected staged files to be detected as tracked changes")
		}
	})
	...
	t.Run("returns true for modified tracked files", func(t *testing.T) {
		...
		testFile := filepath.Join(repo.Path, "test.txt")
		if err := os.WriteFile(testFile, []byte("modified content"), fs.FileStrict); err != nil {
			t.Fatalf("failed to modify tracked file: %v", err)
		}

		hasChanges, err := HasTrackedChanges(repo.Path)
		if err != nil {
			t.Fatalf("HasTrackedChanges failed: %v", err)
		}
		if !hasChanges {
			t.Error("expected modified tracked files to be detected")
		}
	})

	t.Run("returns true for deleted tracked files", func(t *testing.T) {
		repo := testgit.NewTestRepo(t)
		...
		if !hasChanges {
			t.Error("expected deleted tracked files to be detected")
		}
	})

	t.Run("returns true for mixed tracked and untracked changes", func(t *testing.T) {
		repo := testgit.NewTestRepo(t)
		...
		untrackedFile := filepath.Join(repo.Path, "untracked.txt")
		if err := os.WriteFile(untrackedFile, []byte("untracked"), fs.FileStrict); err != nil {
			t.Fatalf("failed to create untracked file: %v", err)
		}
		...
		if !hasChanges {
			t.Error("expected tracked changes to be detected despite untracked files")
		}
	})
	...
	t.Run("fails for non-git directory", func(t *testing.T) {
		...
		_, err := HasTrackedChanges(tempDir)
		if err == nil {
			t.Fatal("HasTrackedChanges should fail for non-git directory")
		}
	})
}
